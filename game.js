...
batonRange:75,batonKnockback:18
};
...
player.damage=12+save.shivLevel*6;
player.attackSpeed=Math.max(12,20-save.shivLevel);
player.batonRange=0;
player.batonKnockback=0;
}else{
player.damage=14+save.batonLevel*5;
player.attackSpeed=Math.max(15,25-save.batonLevel);
player.batonRange=85+save.batonLevel*10;
player.batonKnockback=26+save.batonLevel*6;
}
}
...
function spawnBossGuard(){
let side=Math.floor(Math.random()*4),x,y;
if(side===0){x=-40;y=Math.random()*innerHeight}
if(side===1){x=innerWidth+40;y=Math.random()*innerHeight}
if(side===2){x=Math.random()*innerWidth;y=-40}
if(side===3){x=Math.random()*innerWidth;y=innerHeight+40}
enemies.push({x,y,w:26,h:34,hp:42,speed:2.25,damage:11,tough:false});
}
...
bossSummonTimer=0;
boss={
x:innerWidth/2,y:90,w:54,h:68,
hp:1100,maxHp:1100,
speed:2.15,damage:18,
angle:0,batonRange:76,
phase75:false,phase50:false,phase25:false,
specialTimer:0,rushTimer:0
};
}
...
if(hpPct<=.75&&!boss.phase75){
boss.phase75=true;
boss.batonRange+=18;
boss.speed+=.25;
bossSpecial("RAGE SPIN");
}

if(hpPct<=.50&&!boss.phase50){
boss.phase50=true;
boss.speed+=.55;
boss.rushTimer=260;
bossSpecial("BATON RUSH");
}

if(hpPct<=.25&&!boss.phase25){
boss.phase25=true;
boss.speed+=.35;
boss.damage+=7;
boss.batonRange+=16;
bossSummonTimer=1;
bossSpecial("CALL THE GUARDS");
}

let a=Math.atan2(player.y-boss.y,player.x-boss.x);
let moveSpeed=boss.rushTimer>0?boss.speed*1.9:boss.speed;
boss.x+=Math.cos(a)*moveSpeed;
boss.y+=Math.sin(a)*moveSpeed;

if(boss.rushTimer>0)boss.rushTimer--;

let spinSpeed=boss.specialTimer>0?.33:.16;
...
if(dist(player,boss)<42){
player.hp-=boss.damage*.055;
}

let batonX=boss.x+Math.cos(boss.angle)*boss.batonRange;
let batonY=boss.y+Math.sin(boss.angle)*boss.batonRange;

if(Math.hypot(player.x-batonX,player.y-batonY)<32){
player.hp-=1.7;
let push=Math.atan2(player.y-batonY,player.x-batonX);
player.x+=Math.cos(push)*9;
player.y+=Math.sin(push)*9;
}

for(let b of bullets){
if(!b.dead&&Math.hypot(b.x-boss.x,b.y-boss.y)<42){
boss.hp-=b.damage;
b.dead=true;
}
}
...
function swingBaton(){
batonSwing=11;
let range=player.batonRange;
for(let e of enemies){
if(dist(player,e)<range){
e.hp-=player.damage;
let push=Math.atan2(e.y-player.y,e.x-player.x);
e.x+=Math.cos(push)*player.batonKnockback;
e.y+=Math.sin(push)*player.batonKnockback;
}
}
if(inBoss&&boss&&dist(player,boss)<range){
boss.hp-=player.damage;
let push=Math.atan2(boss.y-player.y,boss.x-player.x);
boss.x+=Math.cos(push)*10;
boss.y+=Math.sin(push)*10;
}
message="Baton swing!";
}

const generalFinds=[
{name:"Bandage Wrap",text:"Heal 35 HP",go:()=>player.hp=Math.min(player.maxHp,player.hp+35)},
...
const shivFinds=[
{name:"Sharpen Shiv",text:"+5 shiv damage this run",go:()=>player.damage+=5},
{name:"Quick Throw",text:"Throw shivs faster",go:()=>player.attackSpeed=Math.max(9,player.attackSpeed-3)},
{name:"Heavy Point",text:"+9 damage but slower throws",go:()=>{player.damage+=9;player.attackSpeed+=3}},
{name:"Long Throw",text:"Shivs fly farther",go:()=>{}}
];

const batonFinds=[
{name:"Heavy Swing",text:"+6 baton damage this run",go:()=>player.damage+=6},
{name:"Wider Swing",text:"+15 baton range",go:()=>player.batonRange+=15},
{name:"Stronger Knockback",text:"Baton pushes enemies farther",go:()=>player.batonKnockback+=14},
{name:"Faster Recovery",text:"Swing baton faster",go:()=>player.attackSpeed=Math.max(12,player.attackSpeed-3)}
];
...
let weaponPool=player.weapon==="shiv"?shivFinds:batonFinds;
let pool=[...generalFinds,...weaponPool];
let picks=[...pool].sort(()=>Math.random()-.5).slice(0,3);
...
addUpgrade("Shiv Training","Permanent shiv damage and speed","shivLevel",30);
if(save.batonPurchased)addUpgrade("Baton Training","Permanent baton damage, range, and knockback","batonLevel",40);
}
...
ctx.lineWidth=5;
ctx.beginPath();
if(batonSwing>0){ctx.arc(player.x,player.y,player.batonRange*.65,0,Math.PI*2)}
else{ctx.moveTo(player.x+12,player.y);ctx.lineTo(player.x+48,player.y+8)}
ctx.stroke();
ctx.lineWidth=1;
}
}
